...
from jinja2modern.default_settings import JINJA2MODERN_ENGINES
from jinja2modern.default_settings import JINJA2MODERN_HOME
from jinja2modern.default_settings import JINJA2MODERN_MEDIA_PATH
from jinja2modern.default_settings import JINJA2MODERN_MEDIA_URL


class Parser():

	def __init__(self, environment, parser_bin, template, out_dir,
		out_extension=None, parser_single=None, parser_multiple=None):
		self.parser_bin = JINJA2MODERN_ENGINES[parser_bin]
		if not os.path.exists(self.parser_bin):
			raise ParserError(f"Parser not found: {self.parser_bin}")
	...
	def render(self, file_link):
		try:
			...
			out_t = self.environment.jinja2modern_loader.load(self.environment,
				self.template)
